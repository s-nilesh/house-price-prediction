...
def test_initialization(model, simple_data):
    ...
    assert model.coefs.shape == (2, 1), (
        f"Expected coefficients shape (2, 1), got {model.coefs.shape}"
    )
...
def test_fit_method(model, simple_data):
    ...
    assert np.all(model.coefs != 0), (
        "Coefficients should not remain zero after training"
    )

    # Check if the model has learned a meaningful relationship (slope should be close to 2)
    assert np.isclose(model.coefs[1], 2, atol=0.1), (
        f"Slope is not close to 2, got {model.coefs[1]}"
    )
    assert np.isclose(model.coefs[0], 1, atol=0.1), (
        f"Intercept is not close to 1, got {model.coefs[0]}"
    )
...
def test_predict_method(model, simple_data):
    ...
    assert y_pred.shape == y_train.shape, (
        f"Expected shape {y_train.shape}, got {y_pred.shape}"
    )
    ...
    np.testing.assert_allclose(
        ...
        y_train,
        atol=1e-1,
        err_msg="Predictions are not close enough to the actual values",
    )
